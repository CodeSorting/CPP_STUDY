...
class ic_engine : public engine { //내연기관 엔진
private:
  virtual void acceleration_output() = 0;
  virtual void reduce_output() = 0;
};

class gs_engine : public ic_engine { //가스엔진
private:
...
class elec_engine : public engine { //전기엔진
private:
...
int main(int argc, char** argv) {
  ...
  gs_engine my_engine; //객체 선언 : gs_engine *my_engine = new ge_engine();도 된다.
  sensor my_sensor;
  break_system my_break_system;
  accelerator my_accelerator(my_engine);
  cruise_controller my_cruise_controller(my_sensor, my_accelerator, my_break_system); //크루즈 컨트롤러 객체 생성
  car my_car(my_engine, my_sensor, my_break_system, my_accelerator, my_cruise_controller); //car 객체 생성
