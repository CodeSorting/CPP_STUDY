...
int main() {
  ...
  monster_b second_mon = first_mon; //직접 대입
...
/*
[30 , 30] 첫번째 몬스터(수수께끼 몬스터 - 100)가 내는 수수께끼 : 문이 뒤집어 지면 무엇이 될까?
[45 , 50] 두번째 몬스터(수수께끼 몬스터 - 100)가 내는 수수께끼 : 문이 뒤집어 지면 무엇이 될까?
위치는 배열이라 깊은 복사로 변경이 두번째 몬스터만 되었고
퀴즈는 얕은 복사라 첫번째 몬스터랑 두번째 몬스터 둘 다 변경됨.
-> 즉, 동적으로 메모리가 할당된 멤버변수의 값은 깊은 복사가 이루어지도록 해야함.
*/
  return 0;
}
