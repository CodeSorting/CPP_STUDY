...
private:
  static list<monster*> mon_a_list; //몬스터 리스트
  static list<monster*> mon_b_list;
  static list<monster*> mon_c_list;
  static int mon_a_count; //몬스터 숫자 (초반에 0으로 초기화)
  static int mon_b_count;
  static int mon_c_count;
};
//전역 멤버 초기화 해야함 (전역 변수,함수는 딱 한번만 밖에서 초기화할 수 있음.)
...
int main() {
  //팩토리 패턴 이용 : 객체를 일정한 규칙으로 생성하고 복잡한 로직을 캡슐화해서 시스템의 확장성을 높이고 변화에 대응하기가 쉽다.
