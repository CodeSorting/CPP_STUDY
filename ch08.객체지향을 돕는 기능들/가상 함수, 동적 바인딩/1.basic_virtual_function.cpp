...
int main() {
  ...
  monster& mon = forest_monster;   // 업캐스팅(부모 클래스에 자식 객체 넣기) 발생
  monster_a& mon_a = forest_monster;

  cout << endl << "부모 클래스로 업캐스팅 후 공격" << endl;
  mon.attack_special(player_1);  // monster_a의 오버라이딩 함수 호출
  //virtual을 선언하지 않을 경우 인텡글 공격(monster_a의 함수)이 아닌 기본 공격(monster의 함수)이 출력된다.

  cout << endl << "자식 클래스로 공격" << endl;
  mon_a.attack_special(player_1);

  cout << endl << "범위 연산자로 공격" << endl;
  mon_a.monster::attack_special(player_1); //얘는 monster 클래스의 함수를 직접적으로 범위 연산자로 요청했으니 기본 공격을 출력한다.
