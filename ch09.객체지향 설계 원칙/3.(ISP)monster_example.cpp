...
/*
기존 몬스터 클래스에 있던 순수 가상 함수들을 이동 인터페이스인 IRoute와 공격 인터페이스인 IAttack으로 분리했다.
이로써 몬스터와 NPC가 이동 인터페이스는 활용하지만, 공격이 필요하지 않은 NPC는 공격 인터페이스를 구현하지 않고 필요한 인터페이스만 구현할 수 있다.
*/
