...
public:
  ...
  void sale_using_lambda_with_this(int payments[], int changes[]) {
    ...
    /*
    **this->**는 클래스의 멤버 변수나 멤버 함수에 접근할 때 사용한다.
    changes는 지역 변수이므로 **changes[i]**로 접근하면 된다.
    this->changes[i]처럼 사용할 필요가 없다.
    price는 멤버변수이므로 this->price[i]를 쓴다.
    */
  };
private:
  int price[loop_count]; //클래스의 멤버 변수이니 changes[]와 다르다.
};
